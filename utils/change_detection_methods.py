...
from utils import paths, geofiles, visualization, sn7_helpers
...
def extract_change_variables(images: np.ndarray, dates: list, parallelize: bool = True) -> np.ndarray:
    ...
    if parallelize:
        ...
        change_variables = Parallel(n_jobs=8)(
            delayed(change_detection_algorithm)(y=imagesT[i, :], x=t) for i in range(n_pixels))
...
if __name__ == '__main__':
    ...
    for i, aoi_id in enumerate(sn7_helpers.get_aoi_ids()):
        if i >= 53:
            ...
            sar_data_file = Path(dirs.DATA) / 'images' / f'{aoi_id}.tif'

            sar_data, transform, crs, date_strings = geofiles.read_tif(sar_data_file)

            change_variables = extract_change_variables(sar_data, date_strings, parallelize=True)
